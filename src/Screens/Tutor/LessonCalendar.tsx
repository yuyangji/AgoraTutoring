...
const exampleAttendanceData: { [Key: string]: Attendance[] } = {
  "2023-08-17": [
...
export const LessonCalendar = () => {
  ...
  const [selectedAttendanceData, setSelectedAttendanceData] = useState<Attendance[] | null>(null);
  const [attendanceData, setAttendanceData] = useState(exampleAttendanceData)

  let markedDay = {};
  Object.keys(exampleAttendanceData).map((item) => {
    markedDay[item] = {
      marked: true,
      dotColor: "orange",
    };
  ...
  const OnPressDay = (dateString: string) => {
    ...
    if (dateString in attendanceData)
      setSelectedAttendanceData(attendanceData[dateString]);
    else setSelectedAttendanceData(null);
  };
